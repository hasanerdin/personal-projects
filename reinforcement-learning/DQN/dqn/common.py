...
def linear_annealing(init_value: float, min_value: float, decay_range: int
    ...
    """
    decay_time = 0
    decay_factor = (min_value - init_value) / decay_range
    value = init_value
    while True:
        yield value
...
def exponential_annealing(init_value: float, min_value: float, decay_ratio
    ...
    Yields:
        ...
        every call
    """
    decay_factor = (min_value / init_value) ** decay_ratio
    decay_value = init_value
    while True:
        yield decay_value

        decay_value *= decay_factor
...
class DerivativeEnv(gym.Wrapper):
    """ 
    Environment wrapper that return the difference 
