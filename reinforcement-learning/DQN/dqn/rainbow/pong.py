...
class ValueNet(torch.nn.Module):
    ...
    def __init__(self, in_channel: int, out_size: int, extensions: Dict[str, Any]):
        ...
        self.conv_layers = torch.nn.Sequential(
            torch.nn.Conv2d(in_channel, 32, kernel_size=8, stride=4),
        ...
        conv_out = self.conv_layers(torch.autograd.Variable(torch.zeros(1, in_channel, 80, 80)))
        linear_in_size = int(np.prod(conv_out.size()))
        ...
        if extensions["noisy"]:
            ...
            linear_layer = lambda in_feature, out_feature: torch.nn.Linear(in_feature, out_feature)

        self.fc1 = linear_layer(linear_in_size, 512)
        self.fc2 = HeadLayer(512, out_size, extensions, 256)
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        out = torch.nn.functional.relu(self.fc1(conv_out))
        out = self.fc2(out)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-iterations", type=int, default=5000000,
                        help="Number of training iterations")
    parser.add_argument("--start-update", type=int, default=40000,
    ...
    parser.add_argument("--eval-episode", type=int, default=4,
                        help="Number of episodes to evaluate")
    parser.add_argument("--save-model", action="store_true",
                        help="If given most successful models so far will be saved")
    parser.add_argument("--model-dir", type=str, default="models/",
                        help="Directory to save models")
    parser.add_argument("--write-period", type=int, default=5000,
                        help="Writer period")
