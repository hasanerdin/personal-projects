...
class Trainer:
    ...
    def update(self, iteration: int) -> None:
        ...
        if iteration >= self.args.start_update:
            ...
            loss = self.agent.loss(sample_batch, self.args.gamma)
    ...
    def __iter__(self) -> Generator[UniformBuffer.Transition, None, None]:
        ...
        for _ in range(self.args.n_iterations):
            ...
            torch_state = self.agent.state_to_torch(current_state, self.args.device).unsqueeze(0)
            action = self.agent.e_greedy_policy(torch_state, eps)

            next_state, reward, done, _ = self.env.step(action)
            episodic_reward += reward

            yield UniformBuffer.Transition(current_state, action, reward, next_state, done)
